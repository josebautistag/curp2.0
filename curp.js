...
(function (global) {
  ...
  * @param {string} apellidoPaterno - Apellido materno.
  ...
  * @param {array} fechaNacimiento - Arreglo con [ día, mes, año ], cada uno como numero.
  ...
  */
  function generaCurp(param) {
    ...
    param.apellidoPaterno = ajustaCompuesto(normalizaString(param.apellidoPaterno.toUpperCase())).trim();

    param.apellidoMaterno = param.apellidoMaterno || "";
    param.apellidoMaterno = ajustaCompuesto(normalizaString(param.apellidoMaterno.toUpperCase())).trim();

    inicial_nombre = extrarInicial(param.nombre);

    vocal_apellido = param.apellidoPaterno.trim().substring(1).replace(/[BCDFGHJKLMNÑPQRSTVWXYZ]/g, '').substring(0, 1);
    vocal_apellido = (vocal_apellido === '') ? 'X' : vocal_apellido;

    primera_letra_paterno = param.apellidoPaterno.substring(0, 1);
    primera_letra_paterno = primera_letra_paterno === 'Ñ' ? 'X' : primera_letra_paterno;

    if (!param.apellidoMaterno || param.apellidoMaterno === "") {
      primera_letra_materno = 'X';
    } else {
      primera_letra_materno = param.apellidoMaterno.substring(0, 1);
    ...
    posicion_14_16 = [
      primerConsonante(param.apellidoPaterno),
      primerConsonante(param.apellidoMaterno),
      primerConsonante(nombre_a_usar)
    ].join('');

    curp = [
      posicion_1_4,
      pad(param.fechaNacimiento[2] - 1900),
      pad(param.fechaNacimiento[1]),
      pad(param.fechaNacimiento[0]),
      param.sexo.toUpperCase(),
      param.estado.toUpperCase(),
      posicion_14_16,
      param.homonimia || (param.fechaNacimiento[2] > 1999 ? 'A' : 0)
    ].join('');
